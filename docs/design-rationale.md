...
We resist type proliferation. The ten v0.3 entity types (`dataset`, `method`, `experiment`, `derivative`, `finding`, `concept`, `standard`, `publication`, `persona`, `organization`) are added only when an artifact has a genuinely distinct lifecycle or reasoning rule — `experiment` (an active paradigm, unlike a static `standard`), `concept` (the research-question layer a program operates at), and the `persona`/`organization` social layer (the who and where behind the work) each cleared that bar. If a candidate new artifact looks like one of the existing ten, it should subclass via convention in the body rather than become a new top-level type.
